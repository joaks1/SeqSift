...
from seqsift.utils.functions import random_str
from seqsift.utils.errors import TempFSError
...
class TempFileSystem(object):
    ...
    def extract_tar_archive(self, path, parent = None):
        if parent is None:
            parent = self.base_dir
        full_parent = self._check_parent(parent)
        archive = tarfile.open(path)
        archive_base_dir = full_parent
        for i, tar_info in enumerate(archive):
            full_path = os.path.join(full_parent, tar_info.name)
            if tar_info.isdir():
                self._register_dir(full_path)
                if i == 0:
                    archive_base_dir = full_path
            if tar_info.isfile():
                self._register_file(full_path)
        archive.extractall(path = full_parent)
        archive.close()
